...
def remove_local_path(path: str):
    """Safely remove a folder, retrying on Windows if files are in use."""
    import shutil, os, time, stat, logging
    ...
    def onerror(func, path_arg, exc_info):
        ...
        try:
            os.chmod(path_arg, stat.S_IWUSR)  # make writable
            func(path_arg)
        except Exception as exc:
            logging.getLogger("task_receiver").warning(f"Failed to remove {path_arg}: {exc}")
            raise

    # Try multiple times in case Windows locks files briefly
    retries = 5
    delay = 0.2  # seconds
    for attempt in range(retries):
        try:
            shutil.rmtree(path, onerror=onerror)
            logging.getLogger("task_receiver").info(f"[CLEANUP] Removed directory: {path}")
            return
        except PermissionError as e:
            if attempt < retries - 1:
                time.sleep(delay)  # wait and retry
            else:
                logging.getLogger("task_receiver").error(f"[CLEANUP] Could not remove {path}: {e}")
                raise
